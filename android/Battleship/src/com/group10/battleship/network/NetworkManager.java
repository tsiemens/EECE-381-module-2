...
public class NetworkManager extends Object {
	...
	private static PrintWriter NiosSocketOutput; // Android to Nios Output
													// Stream
	private static BufferedReader NiosSocketInput; // Nios to Android Input
													// Stream
	...
	private NetworkManager() {
		handler = new Handler(BattleshipApplication.getAppContext()
				.getMainLooper());
	}
	...
	/* Mutators */
	public void setOnNiosSuccessfulSetupListener(
	...
	public void setOnNiosDataReceivedListener(
	...
	public void setupHost() {
		...
		new Thread(serverThread).start(); // waits for a connection & sets the
											// client socket when it finds one
	}
	...
	public void setupAndroidSocket(String ip, int port, boolean isHostBool)
		...
		if (isHost) {
			...
			SetupSocketThread socketSetupThread = new SetupSocketThread(ip,
					port, false);
	...
	public PrintWriter getNiosSocketOutput() {
		if (NiosSocketOutput == null) {
			try {
				NiosSocketOutput = new PrintWriter(new BufferedWriter(
						new OutputStreamWriter(niosSocket.getOutputStream())),
						true);
			} catch (IOException e) {
	...
	public BufferedReader getNiosSocketInput() {
		if (NiosSocketInput == null)
			try {
				NiosSocketInput = new BufferedReader(new InputStreamReader(
						niosSocket.getInputStream()));
			} catch (IOException e) {
	...
	public PrintWriter getAndroidSocketOutput() {
		if (AndroidSocketOutput == null) {
			try {
				AndroidSocketOutput = new PrintWriter(
	...
	public BufferedReader getAndroidSocketInput() {
		if (AndroidSocketInput == null)
			try {
				AndroidSocketInput = new BufferedReader(new InputStreamReader(
					...
					Runnable successfulSetup = new Runnable() {
						...
						public void run() {
							if (onNiosSuccessfulSetupListener != null)
								onNiosSuccessfulSetupListener
										.SetupNiosSuccessfully();
						}
					};
				...
				Runnable ipRunnable = new Runnable() {
					...
					public void run() {
						if (onIPFoundListener != null) {
							onIPFoundListener.onIPFound(getAndroidHostIP(),
									getAndroidHostPort());
						}
					}
				};
					...
					Runnable niosDataReceived = new Runnable() {
						...
						public void run() {
							if (onNiosDataReceivedListener != null)
								onNiosDataReceivedListener
										.ReceivedNiosData(receivedString);
						}
					};
	...
	public class AndroidReceiverThread implements Runnable {
		...
		public void run() {
			...
			Log.d(TAG, "onAndroidDataReceivedListener: "
